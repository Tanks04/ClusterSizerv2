...
SERVER_FIELDS = [
    ...
    "sockets", "cores_per_socket", "threads_per_core", "ram_gb",
    "cpu_frequency", "warranty_expiry",
    "nic_1g", "nic_10g", "nic_25g", "nic_40g", "nic_100g", "nic_fc",
    "notes",
]

STORAGE_FIELDS = [
    "name", "site", "vendor", "model", "raw_capacity_tb",
    "usable_capacity_tb", "raid_overhead_percent", "notes",
]
...
SWITCH_FIELDS = [
    ...
    "ports_1g", "ports_10g", "ports_25g", "ports_40g", "ports_100g", "ports_fc",
    "notes",
]

CONNECTION_FIELDS = [
    "server_name", "switch_name", "speed", "media",
...
def import_connections(
    ...
    for row in _read_rows(path, CONNECTION_FIELDS, "Network Connections"):
        server_uid = server_by_name.get(row.get("server_name", ""))
        switch_uid = switch_by_name.get(row.get("switch_name", ""))

        if not server_uid or not switch_uid:
            skipped += 1
            continue
...
def export_connections(
    ...
    for c in connections:
        rows.append({
            "server_name": server_name_by_uid.get(c.server_uid, "(unknown)"),
            "switch_name": switch_name_by_uid.get(c.switch_uid, "(unknown)"),
            "speed": c.speed,
            "media": c.media,
