...
class ConnectionDialog(QDialog):
    """Server <-> Switch veza. Dropdown-ovi se pune iz trenutnog projekta;
    ispod svakog se prikazuje slobodno/zauzeto za odabranu brzinu, čisto
    informativno (ne blokira spremanje ako je preplaćeno - vidi napomenu u
    calculations/networking.py)."""
    ...
    def __init__(
        ...
        for server in project.servers:
            self.server_combo.addItem(server.name or "(bez imena)", server.uid)
        layout.addRow("Server", self.server_combo)

        self.switch_combo = QComboBox()
        for switch in project.switches:
            self.switch_combo.addItem(switch.name or "(bez imena)", switch.uid)
        ...
        self.port_label_edit.setPlaceholderText("npr. Gi1/0/3, Uplink #1 (opcionalno)")
        ...
        layout.addRow("Trenutno stanje portova", self.usage_label)
    ...
    def _update_usage_hint(self) -> None:
        if not self.project.servers or not self.project.switches:
            self.usage_label.setText("Dodaj barem jedan server i jedan switch.")
            return
        ...
        # Isključi vezu koja se trenutno uređuje iz izračuna, da ne broji samu sebe
        ...
        if server:
            ...
            if server_usage:
                u = server_usage[0]
                parts.append(f"Server {speed}: {u.used}/{u.total} zauzeto")
            else:
                parts.append(f"Server nema deklariranih {speed} NIC-ova (Servers tab)")
        ...
        if switch:
            ...
            if switch_usage:
                u = switch_usage[0]
                parts.append(f"Switch {speed}: {u.used}/{u.total} zauzeto")
            else:
                parts.append(f"Switch nema deklariranih {speed} portova")
